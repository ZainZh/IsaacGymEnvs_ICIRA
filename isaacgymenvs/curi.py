...
def create_attractor():
    ...
    pose.p = gymapi.Vec3(-1.3, 0.0, 0.0)
...
def create_sim(gym, args):
    ...
    cup_asset = gym.load_asset(sim, asset_root, cup_asset_file, other_asset_options)

    # load shelf and spoon
    box_dims = gymapi.Vec3(0.1, 0.04, 0.1)
    other_asset_options.fix_base_link = True
    shelf_asset = gym.load_asset(sim, asset_root, shelf_asset_file, other_asset_options)
    box_asset = gym.create_box(sim, box_dims.x, box_dims.y, box_dims.z, other_asset_options)
    other_asset_options.fix_base_link = False
    spoon_asset = gym.load_asset(sim, asset_root, spoon_asset_file, other_asset_options)
    # Set up the env grid
    num_envs = 1
    spacing = 2.0
    ...
    for i in range(num_envs):
        ...
        # add curi
        curi_handle = gym.create_actor(env, curi_asset, pose, "curi", i, 0)
        ...
        shelf_actor = gym.create_actor(env, shelf_asset, shelf_pose, "shelf", i, 0)
