...
class BaseMultiPlayer(object):
    ...
    def env_step(self, env, actions):
        ...
        if self.is_tensor_obses:
            ...
            if np.isscalar(dones):
                rewards = np.expand_dims(np.asarray(rewards), 0)
                left_rewards = np.expand_dims(np.asarray(left_rewards), 0)
                right_rewards = np.expand_dims(np.asarray(right_rewards), 0)
                dones = np.expand_dims(np.asarray(dones), 0)
    ...
    def get_weights(self):
        weights_left = {}
        weights_right = {}
        weights_left['model'] = self.model_left.state_dict()
        weights_right['model'] = self.model_right.state_dict()
        return weights_left, weights_right

    def set_weights(self, weights_left, weights_right):
        self.model_left.load_state_dict(weights_left['model'])
        self.model_right.load_state_dict(weights_right['model'])
        if self.normalize_input and 'running_mean_std' in weights_left:
            self.model_left.running_mean_std.load_state_dict(weights_left['running_mean_std'])
        if self.normalize_input and 'running_mean_std' in weights_right:
            self.model_right.running_mean_std.load_state_dict(weights_right['running_mean_std'])
    ...
    def run(self):
        ...
        for _ in range(n_games):
            ...
            for n in range(self.max_steps):
                ...
                all_done_indices = done.nonzero(as_tuple=False)
                all_done_indices_cup = done_cup.nonzero(as_tuple=False)
                all_done_indices_spoon = done_spoon.nonzero(as_tuple=False)
                # done_indices = all_done_indices[::self.num_agents]
