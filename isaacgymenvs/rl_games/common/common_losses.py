...
# offline_critic_loss is for adding offline data
def critic_loss_offline(value_preds_batch, values, curr_e_clip, return_batch, clip_value,
                        values_offline, values_random):
    if clip_value:
        value_pred_clipped = value_preds_batch + \
                             (values - value_preds_batch).clamp(-curr_e_clip, curr_e_clip)
        value_losses = (values - return_batch) ** 2
    ...
    c_loss -= values_offline
    c_loss = values_random + c_loss
    return c_loss
